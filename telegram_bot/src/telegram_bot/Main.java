		...
		while (true) {
			...
			for (Update update : updates) {
				...
				if (update.myChatMember() != null) {
					System.out.println("Recebendo atualização: " + update.myChatMember().toString());
				}
				...
				if (update.message() != null) {
					...
					Iteracao interacao = new Iteracao(conversa, update.message().text());					
					...
					if (!respostaBot.isPossuiTeclado()) {
	  					// remove o teclado da mensagem anterior
						baseResponse = bot
								.execute(new SendChatAction(conversa.getIdentificador(), ChatAction.typing.name()));
						sendResponse = bot
	                                 .execute(new SendMessage(conversa.getIdentificador(), "OK").replyMarkup(new ReplyKeyboardRemove()));
						System.out.println("Resposta de Chat Action Enviada? " + baseResponse.isOk());
						System.out.println("Teclado removido? " + sendResponse.isOk());
						...
						int i = 0;
						if (respostaBot.getTexto().size() > 1 ) {
							do {
								baseResponse = bot
										.execute(new SendChatAction(conversa.getIdentificador(), ChatAction.typing.name()));
								sendResponse = bot
								...
								System.out.println("Mensagem Enviada? " + sendResponse.isOk());
								i++;							
								}while (i < respostaBot.getTexto().size() - 1 );
						}
						...
						sendResponse = bot.execute(new SendMessage(conversa.getIdentificador(),
								respostaBot.getTexto().get(i)).replyMarkup(respostaBot.getTeclado()));
						System.out.println("Resposta de Chat Action Enviada? " + baseResponse.isOk());
						System.out.println("Mensagem Enviada? " + sendResponse.isOk());
					}
